...
-- vediamo se abbiamo disponibilit�
...
loop
    ...
    if v_c = v_lun then             -- se il counter ha trovato tutti i giorni disponibili torno la struttura, altrimenti passer� a quella successiva
        v_idts := v_loc;
        exit;
    end if;
...
-- ora vediamo se il cliente non � inserito lo inseriamo

select count(idtc) 
into v_c                            -- anche se l'ho usato prima, non mi riservir� pi� quindi posso riciclare la var
from RES2.tc
where cf = p_cf;

IF v_c = 0 and v_idts <> 0 then  
    INSERT INTO RES2.TC(idtc, CF, NOME) VALUES (RES2.seq_idtc.nextval, p_cf, p_nome);               -- c'� la possibilit� di returning sulla insert
...
RES2.P1('20/12/2023', '18/12/2023', -2, 'ANNA', 'ASIJBDKJA');        -- da strutture al compdeto, non � gestito bene ma almeno non si rompe
END;
/
